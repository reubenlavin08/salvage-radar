...
def upsert_triage(conn: sqlite3.Connection, t: TriageResult) -> None:
    conn.execute(
        ...
        (t.rss_id, datetime.utcnow().isoformat() + "Z", t.item_kind,
...
def upsert_extraction(conn: sqlite3.Connection, e: ExtractionResult) -> None:
    conn.execute(
        ...
        (e.rss_id, datetime.utcnow().isoformat() + "Z", e.item_kind,
         e.overall_condition,
    ...
    conn.execute(
        """INSERT INTO appraisal (rss_id, run_at, ask_price, salvage_low,
           ...
           ON CONFLICT(rss_id) DO UPDATE SET
              ...
              raw_json=excluded.raw_json""",
        (a.rss_id, datetime.utcnow().isoformat() + "Z", a.ask_price,
...
def upsert_rejection(
    ...
    conn.execute(
        ...
        (rss_id, datetime.utcnow().isoformat() + "Z", ask_price,
         0.0, 0.0, 0.0, 0.0,
